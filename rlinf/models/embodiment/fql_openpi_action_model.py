...
from rlinf.models.embodiment.modules.onestep_student import OneStepStudent


@dataclass(frozen=True)
class FQLOpenPi0Config(Pi0Config):
    # config for rl
    ...
    # fql
    normalize_q_loss: bool = False  # norlmaize q_loss for actor loss
    q_chunk_cirtic_input: bool = False  # only for q_head
    q_detach_critic_input: bool = False  # detach q_head input
    q_alpha: float = 1.0   # weight of distill loss in actor loss


class FQLOpenPi0ForRLActionPrediction(BasePolicy, PI0Pytorch):
    """
    Pi0 model for FQL reinforcement learning action prediction.
    """

    config: FQLOpenPi0Config
    ...
    def __init__(
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # FQL compents
        assert getattr(self.config, "add_q_head", False) == getattr(self.config, "add_onestep_head", False), \
                    "FQL requires both 'add_q_head' and 'add_onestep_head' to be set to the same value."
        assert getattr(self.config, "train_expert_only", False) == True, \
                "'train_expert_only must be True in FQL, must freeze VLM of PI0'"

        # action_full_dim = self.config.action_dim * self.config.action_horizon

        # one-step-actor-head, input teacher for reuse encoder_transformer
        if self.config.add_onestep_head:
            self.onestep = OneStepStudent(config=config)

        # double q+target_q head for fql
        if self.config.add_q_head:
            if self.config.config_name == "pi05_maniskill":
                value_head_hidden_sizes = (1024, 512, 256)
            else:
                value_head_hidden_sizes = (512, 256, 128)
            value_head_hidden_sizes = list(value_head_hidden_sizes)
            # Q input is (o, a)
            self.q_head = MultiQHead(
                hidden_size=2048,
                action_feature_dim=self.config.action_dim,
                hidden_dims=value_head_hidden_sizes,
                num_q_heads=2,
                output_dim=1,
    ...
    def forward(self, forward_type="default_forward", **kwargs):
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # FQL use below forward as a offpolicy algorithm
        elif forward_type == "fql_onestep_forward":
            return self.onestep_forward(**kwargs)
        elif forward_type == "fql_teacher_forward":
            return self.teacher_forward(**kwargs)
        elif forward_type == "fql_q_forward":
            return self.fql_q_forward(**kwargs)
        elif forward_type == "fql_target_q_forward":
            return self.fql_target_q_forward(**kwargs)
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        else:
            raise NotImplementedError

    def sft_forward(self, data, **kwargs):
        observation = _model.Observation.from_dict(data["observation"])
        actions = data["actions"]
        # return PI0Pytorch.forward(self, observation, actions)
        return self.bc_flow_forward(observation, actions)
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    @torch.no_grad()
    def soft_update_target_q_head(self, tau: float = 1.0):
        """Soft update target_q_head from q_head
    ...
    def get_state_feature(
        ...
        # transform token-level to time-level [B, N_tokens, D] -> [B, D]
        state_emb = prefix_out.mean(dim=1)
        # align to action horizon [B, D] -> [B, T, D]
        state_feature = state_emb.unsqueeze(1).expand(-1, self.config.action_horizon, -1)

        return state_feature

    # Get prefix out from observation, for onestudent.
    @torch.no_grad()
    ...
    def onestep_forward(self, obs, **kwargs):
        # obs = self.input_transform(obs, transpose=False)
        obs = _model.Observation.from_dict(obs)
        prefix_kv_caches = self.get_prefix_kv_cache(obs)
        action = self.onestep(obs, prefix_kv_caches)
        return action

    def teacher_forward(self, obs, compute_values=False, **kwargs):
        # obs = self.input_transform(obs, transpose=False)
        obs = _model.Observation.from_dict(obs)
        output = self.sample_actions(
            observation=obs,
            noise=None,
    ...
    def fql_q_forward(self, obs, actions, obs_feature=None):
        """Calculate q_head value.
        if input obs_feature, directly use obs_feature as q_head input.
        output obs_feature.
        """
        # obs = self.input_transform(obs, transpose=False)
        obs = _model.Observation.from_dict(obs)
        if obs_feature is None:
            with torch.no_grad():
                obs_feature = self.get_state_feature(obs)
        qf = self.q_head(obs_feature, actions)
        return qf  #, obs_feature

    def fql_target_q_forward(self, obs, actions, obs_feature=None):
        # obs = self.input_transform(obs, transpose=False)
        obs = _model.Observation.from_dict(obs)
        if obs_feature is None:
            with torch.no_grad():
                obs_feature = self.get_state_feature(obs)
        qf = self.target_q_head(obs_feature, actions)
        return qf  #, obs_feature

    def bc_flow_forward(self, observation, actions, noise=None, time=None):
        images, img_masks, lang_tokens, lang_masks, state = \
                self._preprocess_observation(observation, train=False)
        model_type = self.paligemma_with_expert.\
                paligemma.language_model.layers[0].self_attn.q_proj.weight.dtype
        ...
        prefix_att_2d_masks_4d = self._prepare_attention_masks_4d(prefix_att_2d_masks)
        self.paligemma_with_expert.paligemma.language_model.config._attn_implementation = "eager"  # noqa: SLF001
        if model_type == torch.bfloat16:
            prefix_embs = prefix_embs.to(dtype=torch.bfloat16)
            prefix_att_2d_masks_4d = prefix_att_2d_masks_4d.to(dtype=torch.bfloat16)
        ...
        if model_type== torch.bfloat16:
            ...
            full_att_2d_masks_4d = full_att_2d_masks_4d.to(dtype=torch.bfloat16)
        ...
        return F.mse_loss(u_t, v_t, reduction="mean")
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def preprocess_env_obs_extra(self, env_obs):
        # return super().preprocess_env_obs(env_obs)
        processed_obs = self.obs_processor(env_obs)
        policy_obs = self.input_transform(processed_obs, transpose=False)
        return policy_obs
